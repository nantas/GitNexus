...
- [ ] 1.1 已确认 capability spec `cli-analyze-extension-filter` 的 5 个 requirement（extensions-cli-to-pipeline-chain ×3 scenarios，extensions-filter-at-scan-stage ×2 scenarios，interface-contract-unchanged ×2 scenarios）
- [ ] 1.2 无外部依赖；所有修改均在 GitNexus 仓库内部完成

## 2. 核心实现任务

### 2.1 `filesystem-walker.ts` — 核心过滤逻辑

- [ ] 2.1.1 在 `walkRepositoryPaths()` 函数签名中新增 `includeExtensions?: string[]` 可选参数
      - Spec: `interface-contract-unchanged` / `Scenario: existing-callers-unaffected`
      - Design: D1（过滤位置）/ D2（endsWith 匹配）/ D3（空列表语义）
- [ ] 2.1.2 在 glob 结果与 batch stat 循环之间插入扩展名过滤：
      ...
      - Design: D1, D2, D3
- [ ] 2.1.3 添加基于 `mktemp` 的纯内存测试覆盖（`filesystem-walker.test.ts`）：
...
- [ ] 2.2.1 在 `scanPhase.execute()` 中，从 `ctx.options?.includeExtensions` 读取值，传递给 `walkRepositoryPaths()` 的第三个参数
...
- [ ] 2.3.1 在 `AnalyzeOptions` 接口中新增 `includeExtensions?: string[]` 字段，在 `csharpDefineCsproj` 之后
      - Spec: `extensions-cli-to-pipeline-chain`
      - Design: D4（接口兼容）
- [ ] 2.3.2 在 `runPipelineFromRepo()` 调用中（~line 283）增加 `includeExtensions: options.includeExtensions`
      - Spec: `extensions-cli-to-pipeline-chain` / `Scenario: extensions-cli-flag-filters-scan`
      - Design: D4
- [ ] 2.3.3 在 meta.json `analyzeOptions` 持久化对象中（~line 508）增加 `includeExtensions: options.includeExtensions`
      - Spec: `extensions-cli-to-pipeline-chain` / `Scenario: reuse-options-persists-extensions`

### 2.4 `analyze.ts` — CLI 入口

- [ ] 2.4.1 在 `runFullAnalysis()` 调用处（~line 421-440）增加 `includeExtensions: effective.includeExtensions`
      - Spec: `extensions-cli-to-pipeline-chain` / `Scenario: extensions-cli-flag-filters-scan`
      - Design: D4
...
- [ ] 3.2 回写摘要：实施完成后在 `verification.md` 中记录修复链路状态

## 4. 验证与回写收敛

- [ ] 4.1 基于实现结果生成 `verification.md`，逐条确认 spec requirement 实现情况
- [ ] 4.2 基于 verification.md 生成 `writeback.md`
- [ ] 4.3 执行回写：创建 changelog 文档 `docs/changelog/extensions-param-fix.md`
