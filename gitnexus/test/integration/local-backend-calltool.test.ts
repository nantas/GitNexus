  ...
  (handle) => {
    describe('callTool dispatch with real DB', () => {
      ...
      it('context tool projects method-level process participation for class symbols', async () => {
        ...
        expect(result.processes.some((p: any) => p.evidence_mode === 'method_projected')).toBe(
          true,
        );
        expect(result.processes.every((p: any) => ['high', 'medium'].includes(p.confidence))).toBe(
          true,
        );
      ...
      it('impact tool returns upstream dependents', async () => {
        ...
        it('query keeps direct evidence as high confidence when available', async () => {
          ...
          const directHigh = result.process_symbols.find(
            (s: any) =>
              s.process_evidence_mode === 'direct_step' && s.process_confidence === 'high',
          );
        ...
        it('phase1 process_ref readable', async () => {
          ...
          expect(
            out.processes.every((p: any) => p.process_ref && p.process_ref.readable === true),
          ).toBe(true);
        ...
        it('phase1 no opaque heuristic id leak', async () => {
          ...
          expect(
            out.processes.some((p: any) => String(p.id || '').startsWith('proc:heuristic:')),
          ).toBe(false);
        });
        ...
        it('phase5 confidence fields and verification hints', async () => {
          ...
          try {
            ...
            expect(
              on.processes.every((p: any) => ['high', 'medium', 'low'].includes(p.confidence)),
            ).toBe(true);
            expect(
              on.processes.some((p: any) =>
                Object.prototype.hasOwnProperty.call(p, 'verification_hint'),
              ),
            ).toBe(true);
        ...
        it('query no longer injects heuristic rows when processRows is empty', async () => {
          ...
          try {
            ...
            expect(
              result.processes.some((p: any) => p.evidence_mode === 'resource_heuristic'),
            ).toBe(false);
        ...
        it('context no longer injects heuristic rows when processRows is empty', async () => {
          ...
          try {
            ...
            expect(out.processes.some((p: any) => p.evidence_mode === 'resource_heuristic')).toBe(
              false,
            );
        ...
        it('phase3 minimum evidence contract (no project rules => rule_not_matched)', async () => {
          ...
          expect(['rule_not_matched', 'rule_matched_but_evidence_missing']).toContain(
            out.runtime_claim?.reason,
          );
        });
        ...
        it('phase4 hydration policy', async () => {
          ...
          expect(['verified_full', 'failed']).toContain(
            strict.runtime_claim?.verification_core_status,
          );
          ...
          if (
            strict.hydrationMeta?.fallbackToCompact &&
            strict.runtime_claim?.status !== 'failed'
          ) {
        ...
        it('runtime claim core vs adjusted metadata is stable in context()', async () => {
          ...
          expect(out.runtime_claim).toBeDefined();
          expect(['verified_full', 'failed']).toContain(
            out.runtime_claim?.verification_core_status,
          );
          expect(out.runtime_claim?.verification_core_evidence_level).toBeTruthy();
          expect(typeof out.runtime_claim?.policy_adjusted).toBe('boolean');
        ...
        it('returns lifecycle process metadata without breaking legacy fields', async () => {
          ...
          expect(
            queryResult.processes.some((p: any) => p.process_subtype === 'unity_lifecycle'),
          ).toBe(true);
          expect(queryResult.processes.every((p: any) => typeof p.step_count === 'number')).toBe(
            true,
          );
          expect(queryResult.processes.every((p: any) => typeof p.process_type === 'string')).toBe(
            true,
          );
          ...
          expect(
            contextResult.processes.some((p: any) => p.process_subtype === 'unity_lifecycle'),
          ).toBe(true);
          expect(contextResult.processes.every((p: any) => typeof p.step_count === 'number')).toBe(
            true,
          );
        });
      ...
      describe('tool_map and additional callTool tests', () => {
        let backend: LocalBackend;
