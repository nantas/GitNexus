...
/**
 * Parse source code using tree-sitter with an adaptive native buffer size.
 * Uses the shared parser instance (must call loadLanguage() first to set the grammar).
 *
 ...
 */
export const parseContent = (content: string, oldTree?: Parser.Tree): Parser.Tree => {
  if (!sharedParser) throw new Error('Parser not initialized — call loadParser() first');
  const bufferSize = getTreeSitterBufferSize(content);
  return sharedParser.parse(content, oldTree ?? null, { bufferSize });
};
